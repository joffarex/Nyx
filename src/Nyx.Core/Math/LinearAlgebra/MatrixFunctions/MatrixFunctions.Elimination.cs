...
namespace Nyx.Core.Math.LinearAlgebra.MatrixFunctions
{
    internal static partial class MatrixFunctions
    {
        ...
        {
            return Eliminate(input, form).FullMatrix;
        }
        ...
        private static MatrixEliminationResult FindSolution(MatrixEliminationResult result)
        {
            ...
            var solutionState = MatrixSolutionState.None;
            ...
            if (result.AugmentedColumnCount == 1)
            {
                ...
                var pivotRow = 0;
                for (var col = 0; col < result.UnknownsCount; col++)
                {
        ...
        {
            double coeffecient = 1.0 / input[row, col];
